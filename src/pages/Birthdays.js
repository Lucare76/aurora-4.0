...
import { updateBirthday as updateBirthdayNotification } from '../services/birthdaysService';
import {
  FiPlus,
  FiEdit2,
  FiTrash2,
...
function Birthdays() {
  ...
  // Controlla compleanni imminenti e invia notifiche
  useEffect(() => {
    if (birthdays.length > 0 && emailConfigured) {
      checkAndNotify();
    }
  }, [birthdays, emailConfigured]);
  ...
  const checkAndNotify = async () => {
    ...
    for (const birthday of toNotify) {
      ...
      if (result.success) {
        ...
        await updateBirthdayNotification(birthday.id, {
      ...
      <div className="dashboard-content birthdays-page">
        ...
        {!emailConfigured && (
          <div className="config-guide">
            ...
            <ol>
              <li>
                Vai su <a href="https://www.emailjs.com/" target="_blank" rel="noopener noreferrer">
                  EmailJS.com
                </a> e crea un account gratuito
              </li>
              <li>Aggiungi un servizio email (Gmail, Outlook, etc.)</li>
              <li>
                Crea un template email con queste variabili:
                <code>{'{{user_name}}'}</code>, <code>{'{{birthday_name}}'}</code>,{' '}
                <code>{'{{birthday_date}}'}</code>
              </li>
              <li>
                Copia Service ID, Template ID e Public Key nel file{' '}
