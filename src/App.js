...
function DebugApprovalStatus() {
  const { user, userApprovalStatus } = useAuth();
  ...
  return (
    <div style={{
      ...
      <div style={{ 
        marginBottom: '5px',
        color: userApprovalStatus ? '#10b981' : '#ef4444'
      }}>
        <strong>userApprovalStatus:</strong> {userApprovalStatus ? '✅ EXISTS' : '❌ NULL'}
      </div>
      ...
      {userApprovalStatus && (
        <>
          <div style={{ 
            marginBottom: '5px',
            color: userApprovalStatus.approved ? '#10b981' : '#ef4444'
          }}>
            <strong>approved:</strong> {userApprovalStatus.approved ? '✅ TRUE' : '❌ FALSE'}
          </div>
          
          <div style={{ marginBottom: '5px' }}>
            <strong>status:</strong> {userApprovalStatus.status || 'N/A'}
          </div>
          
          <div style={{ marginBottom: '5px' }}>
            <strong>exists:</strong> {userApprovalStatus.exists ? '✅ YES' : '❌ NO'}
          </div>
          
          <div style={{ fontSize: '10px', marginTop: '5px', color: '#fbbf24' }}>
            <strong>message:</strong> {userApprovalStatus.message}
          </div>
        </>
      )}
      ...
      <div style={{ 
        ...
        {!userApprovalStatus && '⚠️ userApprovalStatus è NULL - problema in AuthContext'}
        {userApprovalStatus && userApprovalStatus.approved && '✅ Utente già approvato'}
        {userApprovalStatus && !userApprovalStatus.approved && '🔴 Utente NON approvato - dovrebbe vedere pending'}
      </div>
    </div>
  );
}
